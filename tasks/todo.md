...
- [x] **Esportazione Immagine (Lista Modem) & Share Nativo**: Implementata una funzione pixel-perfect in `export.js` per generare un'immagine PNG dinamica della griglia dati speculare al formato Excel. Mostra la tabella dei materiali per ciascun tecnico, evidenziando il totale (incluso l'usato) ed i totali finali di riga, inserendo la data reale corretta al posto di "Oggi" generico (es. `14/07/2026` / `14-07-2026`) sia a video che nel nome file (es. `Lista_Modem_Elecnor_14-07-2026.png`). Sulle versioni PWA/Mobile, se supportato, avvia direttamente la condivisione nativa del file (tramite Web Share API) per inviarla rapidamente su WhatsApp o Telegram, mentre su desktop PC si attiva direttamente il salvataggio locale (download).
- [x] **Ottimizzazione layout header PWA**: Spostato il selettore di data (`.snapshot-dropdown`) all'interno di `.content-header-main` di fianco al titolo dell'appalto su mobile (dove c'era lo spazio vuoto indicato). Questo lascia la riga inferiore `.content-actions` interamente dedicata ai tre pulsanti d'azione (Esporta, Stampa, Immagine), che ora si distribuiscono in modo fluido e simmetrico occupando il 100% della larghezza ed evitando che il pulsante Immagine venga tagliato fuori dallo schermo.
...
## 2026-07-31 — Sessione Richiesta Sync Appalto & Nascondi Lista per Admin
- [x] **Pulsante "Richiedi Sync Appalto" nel Drawer Admin (`index.html`, `auth.js`, `data.js`):** Aggiunto il pulsante riservato all'Admin nel menu laterale, posizionato sopra la sezione "Ricerca PFS" e separato da un divider. Cliccandolo, invia una richiesta su Firestore per scatenare il sync istantaneo dell'appalto selezionato sui telefoni Android dei tecnici.
- [x] **Gestione "Nascondi / Riabilita Lista" Tecnico (`data.js`):** Aggiunto il tasto azione dedicato sopra al nome di ciascun tecnico nella tabella dei materiali (visibile unicamente all'Admin). Per gli utenti comuni (Non-Admin), la colonna del tecnico nascosto scompare del tutto. Per la vista Admin, la colonna rimane visibile con opacità ridotta (`opacity: 0.45`), lo stile disabilitato ed il tasto "Riabilita Lista" per ripristinarla.
- [x] **Separazione Nascosti Giornalieri & Protezione Snapshot Passati (`data.js`):** 
  - Sostituito il salvataggio su `hidden_tecnici` (permanenti) con il documento Firestore `settings/hidden_lists_daily` legato alla data specifica (`today` o `YYYY-MM-DD`). Il nascondimento di una lista è ora isolato alla sola giornata/snapshot selezionato e si resetta automaticamente a mezzanotte evitando che un tecnico rimanga disabilitato a vita per dimenticanza.
